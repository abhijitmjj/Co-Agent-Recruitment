...
async def test_session_reuse():
    ...
    response1 = await runner.run_async(
        user_id=user_id, query="who are you?", session_id=session_id
    )
    ...
    response4 = await runner.run_async(
        user_id="different_user", query="who are you?", session_id=session_id
    )
    ...
    response5 = await runner.run_async(
        user_id=user_id, query="who are you?", session_id=None
    )
    logger.info(f"No session ID response length: {len(response5)}")

    logger.info(
        "✅ Session reuse test completed. Check logs for 'Reusing existing session' vs 'Created session' messages."
    )
