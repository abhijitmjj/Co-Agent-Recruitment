...
async def demonstrate_output_key_concept():
    ...
    print(f"   - Purpose: Stores analyzed job posting data in session.state['job_posting_JSON']")
    print()
    ...
    session, _ = await get_or_create_session("co_agent_recruitment", "test_user", session_service=_shared_session_service)
    ...
    print("1. Automatic Storage: Agent outputs are automatically saved to session state")
    print("2. Template Access: Other agents can access data using {key} syntax in instructions")
    print("3. Persistent State: Data persists throughout the session for multi-step workflows")
