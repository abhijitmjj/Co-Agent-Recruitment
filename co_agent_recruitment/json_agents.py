...
async def analyze_job_posting_json(job_posting_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if isinstance(result, dict):
            ...
            if 'error' in result:
                logging.warning(f"Job posting analysis returned error: {result['error']}")
                return result
            else:
...
async def process_document_json(document_text: str, document_type: str = "auto") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if document_type == "resume":
            ...
            job_keywords = [
                "responsibilities", "requirements", "qualifications", "we are seeking",
                "job description", "position", "role", "company", "salary", "benefits",
                "apply", "hiring", "candidate", "experience required"
            ]
            
            # Keywords that suggest it's a resume
            resume_keywords = [
                "education", "work experience", "skills", "projects", "certifications",
                "objective", "summary", "achievements", "accomplishments"
            ]
