...
async def simulate_curl_request():
    ...
    try:
        ...
        result = await process_document_json(
            request_data["document_text"], request_data["document_type"]
        )
        ...
        error_response = {"success": False, "error": f"Invalid input: {str(e)}"}
        ...
        print(f"❌ Exception: {e}")
        error_response = {"success": False, "error": "Failed to process document"}
...
async def test_working_server_instructions():
    ...
    print('     -H "Content-Type: application/json" \\')
    print(
        '     -d \'{"document_text": "As an Engineering Manager...", "document_type": "auto"}\''
    )
